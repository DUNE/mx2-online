...
class LIBox:
	...
	def __init__(self, pulse_height = 12.07, pulse_width = 7, LED_groups = "ABCD", trigger_internal = False, trigger_rate = None):
		...
		self.permitted_commands = ( "_X" , "aA", "aE[0a-vA-V]", "aB[0-3]", "aC[\da-eA-E]_[\da-eA-E]", "aO", "aD[0-7]", "aQ", "aK", "aH[\da-eA-E]{2}", "aI[\da-eA-E]{2}" )
	...
	def write_configuration(self):
		...
		inverseAddress = 0
		for char in "ABCD":
			inverseAddress += 2 ** (self.LED_groups.count(char))
		...
		highBit = highBit.hex().replace("0x", "")
		...
		lowBit1 = lowBit1.hex().replace("0x", "")
		lowBit2 = lowBit % 16;
		lowBit2 = lowBit2.hex().replace("0x", "")
		...
		if (self.trigger_internal):
			...
			self.trigger_rate = int(self.trigger_rate)		# no decimals.
			...
			highNum2 = (trigger_rate - highNum1) / 0x100
			trigger_rate -= highNum1 + highNum2
			
			highNum1 = highNum1.hex().replace("0x", "")
			highNum2 = highNum2.hex().replace("0x", "")
			command_stack.append("aH" + highNum1 + highNum2)
			
			lowNum1 = trigger_rate / 0x10
			lowNum2 = trigger_rate - lowNum1
			lowNum1 = lowNum1.hex().replace("0x", "")
			lowNum2 = lowNum2.hex().replace("0x", "")
			command_stack.append("aI" + lowNum1 + lowNum2)
		else:
...
if __name__ == "__main__":
	...
	parser.add_option("-p", "--pulse-height", dest="pulse_height", help="Set the pulse height (volts).  Valid values: 4.05-12.07.  Default: %default.", default=12.07)
	parser.add_option("-w", "--pulse-width", dest="pulse_width", help="Set the pulse width.  Valid values: 0-7 (roughly corresponding to 20-35 ns).  Default: %default.", default=7)
	...
	parser.add_option("-i", "--trigger-internal", dest="trigger_internal", action="store_true", help="Use internal triggering for the LI pulses.  (You should also set -r if you use this option.)  Default: %default", default=False)
	parser.add_option("-r", "--trigger-rate", dest="trigger_rate", help="Set the triggering rate for internal triggering.  (You should only use this option in conjunction with -i.)  Default: None.", default=None)
