...
class SCApp(wx.App):
    ...
    def __init__(self, theArgs=[]):
        try:
            ...
            self.Bind(wx.EVT_BUTTON, self.OnCHEbtnResetThisCHE, self.frame.che.CHEResets.btnResetThisCHE)
            self.Bind(wx.EVT_BUTTON, self.OnCHEbtnResetThisCROCE, self.frame.che.CHEResets.btnResetThisCROCE)
            self.Bind(wx.EVT_BUTTON, self.OnCHEbtnResetThisCRATE, self.frame.che.CHEResets.btnResetThisCRATE)
            self.Bind(wx.EVT_BUTTON, self.OnCHEbtnResetAllCRATEs, self.frame.che.CHEResets.btnResetAllCRATEs)
...
##            self.Bind(wx.EVT_BUTTON, self.OnFEDAQbtnAcqCtrlStop, self.frame.fe.daq.btnAcqCtrlStop)
    ...
    def LoadHardware(self, useChRst):
        ...
        try:
            ...
            if self.scs!=[]:
                print '\n'.join(['Found '+sc.Description()+' SWRelease='+sc.controller.SWRelease()+\
                    ' BoardFWRelease='+sc.controller.BoardFWRelease() for sc in self.scs])
            #2. Find controller's objects lists
            for sc in self.scs:
                sc.FindCRIMs()
                sc.FindCROCs()
                sc.FindCROCEs()
                sc.ConfigCROCEsREFE(self.verbose)
                sc.FindDIGs()
                if sc.vmeCRIMs!=[]:
    ...
    def OnMenuActionsAbout(self, event):
        try:
            ...
            print '* Version : 2.0.4. Release Date: August 23rd 2013                              *'
            print '*------------------------------------------------------------------------------*'
            ...
            for theCROCE in self.scs[self.frame.vme.crateNumber].vmeCROCEs:
                ...
                for itry in range(ntry):
                    ...
                    if self.frame.vme.BoardTest.chkTest4.IsChecked():
                        fails[4]=self.TestWSRfrmsFIFO(itry,theCROCE,fails[4],self.daqWFile,bitErrFreqCHXTest4,nwords16=505,nsend=128,#505,128
...
##                        if fails[8]!=0: return
                    ...
                    if self.frame.vme.BoardTest.chkTest9.IsChecked():
                        ...
                        for iche in range(4):
                            ...
                            theCROCE.Channels()[iche].WriteConfiguration((0x0BC0&cfg)|(0xA000+nfebs))    #3
                        ...
                        for iche in range(4):
                            ...
                            if (0x0200&statusFrame)!=0x0000:
                                errmsg='TRY#%s, TEST#9.5: Sequencer, %s:%s: ERROR StatusFrame=%s, found SequencerErrorBit set'%(itry,
                                    theCROCE.Description(),theCROCE.Channels()[iche].Description(),hex(statusFrame))
                                fails[9]=fails[9]+1
                                if self.daqWFile!=None: self.daqWFile.write('\n'+errmsg)
                                else: print errmsg
                            if statusTXRX!=0x2570: 
                                errmsg='TRY#%s, TEST#9.6: Sequencer, %s:%s: ERROR StatusTXRX=%s, should be 0x2570'%(itry,
                                    theCROCE.Description(),theCROCE.Channels()[iche].Description(),hex(statusTXRX))
                                fails[9]=fails[9]+1
                                if self.daqWFile!=None: self.daqWFile.write('\n'+errmsg)
                                else: print errmsg
                            if rcvMemWPointer!=nfebs*60:
                                errmsg='TRY#%s, TEST#9.7: Sequencer, %s:%s: ERROR RcvMemWPointer=%s, should be %s'%(itry,
...
##                            theCROCE.Channels()[iche].WriteHeaderData(0x0000)
    ...
    def TestFastCommands(self,itry,theCROCE,fails,daqWFile):
        ...
        for iche in range(4):
            theCROCE.Channels()[iche].WriteCommands(SC_Util.CHECmds['ClearStatus']+SC_Util.CHECmds['ClearRDFECounter'])
            ##theCROCE.Channels()[iche].WriteConfiguration(0x0000) # either keep track of config (RE/FE) or just don't destroy it
            theCROCE.Channels()[iche].WriteHeaderData(0x0000)
        return nfails
    def TestRSTTP(self,itry,theCROCE,fails,daqWFile,testTP,testRST,enCROCE,enCHE,timeout=10):
        ...
        for iche in range(4):
            theCROCE.Channels()[iche].WriteCommands(SC_Util.CHECmds['ClearStatus']+SC_Util.CHECmds['ClearRDFECounter'])
            theCROCE.Channels()[iche].WriteConfiguration(0xFC0F&theCROCE.Channels()[iche].ReadConfiguration())
            #theCROCE.Channels()[iche].WriteHeaderData(0x0000)
        return nfails
    def TestWSRfrmsFIFO(self,itry,theCROCE,fails,daqWFile,bitErrFreqCHX,nwords16=508,nsend=127,dw='D16',
            useBLT=False,useRAMMode=True,useRandom=False,useErrorData=False,theRAMData=0xFFFF,theREGHeader=0x1234,timeout=100):
        ...
        if useRandom==True  and useErrorData==False:
            ...
            ##print 'random: itry=%d, msgsnd=%s'%(itry,''.join([hex(w16)[2:].rjust(4,'0') for w16 in msgsnd]))
        if useRandom==False and useErrorData==True:
            ...
            msgsnd.append(0x0000) #SEE above CAUTION for D32 compliance
            nwords16=len(msgsnd)  #overwrite nwords16 to 133
            nsend=1 #133             #overwrite nsend to maximum value, which is 133=nwords16
            #CHECK:133*(10+2*133) = 36708 bytes = 0x08F64 < 0x1FFFF=131071 bytes (message length is 10+2*133=276bytes % 4 == 0, good)
            #RcvMemWPointer should be 36708 and RcvMemFrmCounter should be 473
        ...
        #This is the loop through all FOUR ChannelE 
        for iche in [0,1,2,3]:
            ...
            for isend in range(nsend):
                ...
                for i in range(timeout):
                    data=theCROCEChannelE.ReadAllRegisters()
                    if (data==[config,0x0000+((0x10000&((10+2*nwords16)*(isend+1)))>>1),isend+1,0x1010,0x2410,
                        0xFFFF&((10+2*nwords16)*(isend+1)),theREGHeader]): break
                if (data!=[config,0x0000+((0x10000&((10+2*nwords16)*(isend+1)))>>1),isend+1,0x1010,0x2410,
                        0xFFFF&((10+2*nwords16)*(isend+1)),theREGHeader]):
            ...
            for repeat in range(repeatRead):
                ...
                for isend in range(data[2]): #nsend frames, data[2]==RRcvMemFramesCounter
                    ...
                    if dw=='D16':
##                        #this is old code
##                        rcvfrmsDataLengthBytes =theCROCEChannelE.ReadReceiveMemory(ibyte,dw='D16')
##                        rcvfrmsStatus          =theCROCEChannelE.ReadReceiveMemory(ibyte+2,dw='D16')
##                        rcvfrmsFirmwareDevFunc =theCROCEChannelE.ReadReceiveMemory(ibyte+4,dw='D16')
##                        rcvfrmsSourceID        =theCROCEChannelE.ReadReceiveMemory(ibyte+6,dw='D16')
##                        rcvfrmsDataLengthBytes2=theCROCEChannelE.ReadReceiveMemory(ibyte+8,dw='D16')
##                        if useBLT==False:
##                            for i16word in range(ibyte+10, ibyte+rcvfrmsDataLengthBytes, 2):
##                                rcvfrmData.append(theCROCEChannelE.ReadReceiveMemory(i16word,dw='D16'))
##                        if useBLT==True:
...
##                                rcvfrmData.append((data16[i+0]<<8) | data16[i+1])
                        #this is new code 07.17.2013
                        rcvfrmsDataLengthBytes=theCROCEChannelE.ReadReceiveMemory(ibyte,dw='D16')
                        if useBLT==False:
                            for i16word in range(ibyte, ibyte+rcvfrmsDataLengthBytes, 2):
                                rcvfrmData.append(theCROCEChannelE.ReadReceiveMemory(i16word,dw='D16'))
                        if useBLT==True:
                            raise Exception('D16 BLT readout mode NOT supported in CROCE')
                        rcvfrmsDataLengthBytes =rcvfrmData[0]
                        rcvfrmsStatus          =rcvfrmData[1]
                        rcvfrmsFirmwareDevFunc =rcvfrmData[2]
                        rcvfrmsSourceID        =rcvfrmData[3]
                        rcvfrmsDataLengthBytes2=rcvfrmData[4]
                        rcvfrmData=rcvfrmData[5:]
                    #7.2. Case D32 
                    if dw=='D32':
##                        #this is old code
...
##                            rcvfrmData=rcvfrmData[:((rcvfrmsDataLengthBytes-10)/2)] #truncate for real data because of the above "CAUTION on VME D32"
                        ...
                        rcvfrmsDataLengthBytes=theCROCEChannelE.ReadReceiveMemory(ibyte,dw='D16')
                        if useBLT==False:
                            for i32word in range(ibyte, ibyte+rcvfrmsDataLengthBytes, 4):
                                data32=theCROCEChannelE.ReadReceiveMemory(i32word,dw='D32')
                                rcvfrmData.append((data32&0xFFFF0000)>>16)
                                rcvfrmData.append((data32&0x0000FFFF))
                        if useBLT==True:
                            data32=theCROCEChannelE.controller.ReadCycleBLT(theCROCEChannelE.RRcvMemory|(ibyte),rcvfrmsDataLengthBytes,am='A32_U_BLT',dw='D32sw')
                            for i in range(0,len(data32),4):
                                rcvfrmData.append((data32[i+0]<<8) | data32[i+1])
                                rcvfrmData.append((data32[i+2]<<8) | data32[i+3])
                        rcvfrmsDataLengthBytes =rcvfrmData[0]
...
##                        nfails=nfails+1
    ...
    def OnFEFPGAbtnRead(self, event):
        try:
            if self.frame.fe.TopLabels[2].GetValue()=='CH' and self.frame.fe.TopLabels[4].GetValue()=='CROC':
                theCROCX=FindVMEdev(self.scs[self.frame.fe.crateNumber].vmeCROCs, self.frame.fe.crocNumber<<16); theType='CROC'
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                theCROCX=FindVMEdev(self.scs[self.frame.fe.crateNumber].vmeCROCEs, self.frame.fe.crocNumber<<24); theType='CROCE'
    ...
    def OnFEFPGAbtnWriteALLThisCRATE(self, event):
        try:
            sentMessageData=FEB(self.frame.fe.febNumber).ParseFPGARegsToMessage(self.frame.fe.fpga.Registers.txtRegs)
            if self.frame.fe.TopLabels[2].GetValue()=='CH' and self.frame.fe.TopLabels[4].GetValue()=='CROC':
                vmeCROCXs=self.scs[self.frame.fe.crateNumber].vmeCROCs; theType='CROC'
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                vmeCROCXs=self.scs[self.frame.fe.crateNumber].vmeCROCEs; theType='CROCE'
        ...
        try:
            ...
            if dlg.ShowModal()==wx.ID_OK:
                ...
                for iPage in range(Flash.NPages):
                    pageBytes=theFEB.FLASHMainMemPageRead(theCROCXChannelX, iPage, theType, dw='D32', useBLT32=True)
                    for iread in range(10):
                        pageBytesIRead=theFEB.FLASHMainMemPageRead(theCROCXChannelX, iPage, theType, dw='D32', useBLT32=True)
                        if pageBytes!=pageBytesIRead:
                            ...
                            for index in range(len(pageBytes)):
                                if pageBytes[index]!=pageBytesIRead[index]:
                                    print'@ index=0x%d, correct=%s, error=0x%s'%(
                                        index,hex(pageBytes[index])[2:].rjust(2,'0'),hex(pageBytesIRead[index])[2:].rjust(2,'0'))
    ...
    def OnFEFLASHbtnCompareFileToFlash(self, event):
        try:
            ...
            if dlg.ShowModal()==wx.ID_OK:
                ...
                errPages=''
                for iPage in range(Flash.NPages):
                    pageBytesRead=theFEB.FLASHMainMemPageRead(theCROCXChannelX, pagesAddrFile[iPage], theType, dw='D32', useBLT32=True)
    ...
    def OnFEFLASHbtnWriteFileToFlash(self, event):
        try:
            ...
            theFEB.WriteFileToFlash(theCROCXChannelX=theCROCXChannelX, theCROCX=theCROCX, theVMECROCXs=None,
                ...
                theFrame=self.frame, theType=theType, dw='D32', useBLT32=True)
        ...
        if theDataType23Hits==True:
            for itimeout in range(100):
                ...
                if ((rcvMessageData[27]>>7)&0x01)==1: #message word 27-bit 7: R  Digitization Done, 1 bit
                    return 
                if itimeout==99:
                    raise Exception('FEDAQWaitDigitizerDoneBit ERROR: timeout=%s %s'%(itimeout,theFEB.FPGADescription(theCROCXChannelX, theCROCX, theType)))
    ...
    def OnFEDAQbtnSoftRDFEWrite(self, event):
        try:
            ...
            if theReadType==0 or theReadType==1:
                ...
                theConfigs=[]
                rdfeCounters=[]
                theConfigs.append(theCROCXChannelX.ReadConfiguration())
                ...
                theCROCXChannelX.WriteConfiguration(0x03FF&theConfigs[0])                                           #7
                ...
                for ich in range(4):
                    ...
                    theCROCX.Channels()[ich].WriteConfiguration(0x03FF&theConfigs[ich])                             #7
                print 'OnFEDAQbtnSoftRDFEWrite END  : %s'%time.ctime()
            elif theReadType==3:
                ...
                for theCTRLX in self.scs[self.frame.fe.crateNumber].vmeCROCEs:
                    ...
                    for ich in range(4):
                        ...
                        SC_MainMethods.DAQReadRcvMemory(rdfeCounters[ich]+1, theCTRLX, theCTRLX.Channels()[ich], theType, vmeCROCXs,
                            vmeCROCXsAllCRATEs, 1, theWriteType, self.daqWFile, self.frame, theDataType23Hits)      #6
                        theCTRLX.Channels()[ich].WriteConfiguration(0x03FF&theConfigs[ich])                         #7
                print 'OnFEDAQbtnSoftRDFEWrite END  : %s'%time.ctime()
            elif theReadType==4:
