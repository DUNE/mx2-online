...
class SCApp(wx.App):
    ...
    def __init__(self, theArgs):
        try:
            ...
            self.debug=False
            self.verbose=False
#            self.version='v:2.0.14.'
            self.version='v:2.0.15.'
##            for x in theArgs:
...
##                if x=='v': self.verbose=True
            ...
            self.Bind(wx.EVT_BUTTON, self.OnCROCEbtnFLASHReloadThisCROCE, self.frame.croce.FLASHReload.btnWrite)
            self.Bind(wx.EVT_BUTTON, self.OnCROCEbtnFLASHReloadThisCrate, self.frame.croce.FLASHReload.btnRead)
...
##            self.Bind(wx.EVT_BUTTON, self.OnFEDAQbtnAcqCtrlStop, self.frame.fe.daq.btnAcqCtrlStop)
    ...
    def PostToECL(self, theCROCEChannelE):
          ...
          #out='This is an automated message. Find Hardware with CHAIN RESET has been performed by '+str(username)+'@'+str(hostname)
          out3= str(username)+'@'+str(hostname)
          out2 = "\n"+"This is an automated posting by "+str(username)+"@"+str(hostname)+" on "+st+". \n"+"A chain reset has been performed in SlowControl on crate "+str(self.frame.che.crateNumber)+" - CROCE "+str(self.frame.che.croceNumber)+" - chain "+str(theCROCEChannelE.cheNumber)+". \n"+"The last run/subrun information is from /work/conditions/last_trigger.dat \n"+lines[0]+lines[1]+"\n"
          e = ECLEntry(category='Automated_Entries/SlowControl',
              tags=['Auto-Entry'],
              formname='default',
              text=out2, #"""This is beginning of the rise of the machines""",
              preformatted=True)
          elconn = ECLConnection(url="http://dbweb5.fnal.gov:8080/ECL/minerva", username="minerva_auto", password="axial2009")
          #
          ...
          #lines = runfile.readlines()
          # LETS SEND AN EMAIL TO PEOPLE. 

          sender = "minerva"
          recievers = "nuruzzaman42@gmail.com","nur@fnal.gov"
          body = MIMEText(out2)
          body['Subject'] = "Slowcontrol: Performed A Chain Reset"
#          body['From'] = "minerva@mnvtb04.fnal.gov"
          body['From'] = out3
          stmp = smtplib.SMTP()
          stmp.connect()
          stmp.sendmail(sender,recievers, body.as_string())
          stmp.quit()
    ...
    def OnMenuActionsAbout(self, event):
        try:
            ...
            print('* Version : %s Release Date: April 5 2017                               *'%(self.version))
            print('*------------------------------------------------------------------------------*')
            print('* NOTES: Version 2.0.15. Release Date: April 5 2017                            *')
            print('* Added function to send email to experts and make an ECL entry whenever there *')
            print('* is ChainRESET or Find Hardware with ChainRESET is performed.                 *')
            print('*------------------------------------------------------------------------------*')
...
##        return nfails
    ...
    def OnCROCEbtnWriteRDFEPulseDelay(self, event):
        try:
            ...
            data = self.frame.croce.RDFESetup.choiceRDFEDelayEnable.GetSelection()<<15 | \
                int(self.frame.croce.RDFESetup.txtRDFEPulseDelayValue.GetValue()) & 0x03FF
    ...
    def OnCROCEbtnReadRDFEPulseDelay(self, event):
        try:
            ...
            self.frame.croce.RDFESetup.txtRDFEPulseDelayValue.SetValue(str(data & 0x03FF))
    ...
    def OnCROCEbtnFLASHReloadThisCROCE(self, event):
        try:
            theCROCE=FindVMEdev(self.scs[self.frame.croce.crateNumber].vmeCROCEs, self.frame.croce.croceNumber<<24)
            theCROCE.SendFLASHReload()
        except: ReportException('OnCROCEbtnFLASHReloadThisCROCE', self.reportErrorChoice)
    def OnCROCEbtnFLASHReloadThisCrate(self, event):
        try:
            theSC=self.scs[self.frame.croce.crateNumber]
            for theCROCE in theSC.vmeCROCEs:
                theCROCE.SendFLASHReload()
        except: ReportException('OnCROCEbtnFLASHReloadThisCrate', self.reportErrorChoice)
    ...
    def OnCROCEbtnWriteFileToFlash(self, event):
        try:
            message='*****!!!!! DANGER !!!!!*****\nIf you encounter errors during this operation\n, DO NOT power cycle or reload the FLASH'
            caption='CROCE CONTROL FLASH WRITE'
...
##                (iPage,i,status,time.time()-start_time)
    ...
    def OnCHEbtnResetThisCHE(self, event):
        try:
            ...
            self.PostToECL(theCROCEChannelE)
...
##        print 'CHEWriteFileToFlash STOP : %s %s'%(time.ctime(),source)
    def OnCHEbtnWriteFileToFlash(self, event):
        try:
            ...
            message='*****!!!!! DANGER !!!!!*****\nIf you encounter errors during this operation\n, DO NOT power cycle or reload the FLASH'
            caption='CROCE CHANNEL FLASH WRITE'
            dlgYes = wx.MessageDialog(self.frame, message, caption, wx.YES_NO | wx.ICON_QUESTION)
            if dlgYes.ShowModal() == wx.ID_YES:
                theCROCE=FindVMEdev(self.scs[self.frame.che.crateNumber].vmeCROCEs, self.frame.che.croceNumber<<24)
                theCROCEChannelE=theCROCE.Channels()[self.frame.che.cheNumber]
