...
class SC():
    ...
    def PostToECL(self,theCROCE, theCROCEChannelE):
        ...
        out2 = "This is an automated posting by "+str(username)+"@"+str(hostname)+" on "+st+" . \n"+"Find Hardware with ChainRESET has been performed in SlowControl. \n"+"RESET issued for: \n"+str(self.Description())+","+str(theCROCE.Description())+", Chain:"+str(theCROCEChannelE.cheNumber)+". \n"+"The last run/subrun info is from /work/conditions/last_trigger.dat :  \n"+lines[0]+lines[1]
...
#                    Subject: THIS IS A TEST-CHAIN RESET"""+out2
#        stmp = smtplib.SMTP()
#        stmp.connect()
#        stmp.sendmail(sender,recievers,message)
        body = MIMEText(out2)
        body['Subject'] = "Slowcontrol: Performed A Find Hardware with ChainRESET"
#        body['From'] = "minerva@mnvtb04.fnal.gov"
                ...
                for chtry in range(1,3):
                    ...
                    if theCROCEChannelE.FEBs!=[]:
                        ...
                        if useChRst==True:
                            ...
                            self.PostToECL(theCROCE,theCROCEChannelE)
    ...
    def HWcfgFileSave(self, f):
        ...
        #Use theFEB.FPGADumpRead which is independent of the Frame.NRegsFPGA
        for theCROCE in self.vmeCROCEs:
            for theCROCEChannelE in theCROCE.Channels():
                for febAddress in theCROCEChannelE.FEBs:
                    theFEB=SC_MainObjects.FEB(febAddress)
                    f.write('%s:%s\n'%(theFEB.FPGADescription(theCROCEChannelE, theCROCE, theType='CROCE'), \
                        [hex(val)[2:].rjust(2,'0') for val in theFEB.FPGADumpRead(theCROCEChannelE, theType='CROCE', theIncludeCRC=theCROCE.includeCRC)[0]]))
        for theCROCE in self.vmeCROCEs:   
            for theCROCEChannelE in theCROCE.Channels():
                for febAddress in theCROCEChannelE.FEBs:
                    theFEB=SC_MainObjects.FEB(febAddress)
    ...
    def HWcfgFileLoad(self, f, theframe=None, scsNumbers=1):
        ...
        for line in f:
            ...
            if int(addresses[-1])==self.boardNum:
                ...
                if lineList[0]==SC_Util.VMEdevTypes.FPGA:
                    nregs=0
                    if int(lineList[2][2+32*6:4+32*6],16)==96:
                        nregs=59 #in v96 the fpga has 59 registers
                    else:
                        nregs=55 #in v95 the fpga has 55 registers
                    if len(lineList[2])<6*nregs: raise Exception('Wrong format in line %s wrong data field length <> 331'%line)
                    if len(addresses)!=4: raise Exception('Wrong format in line %s wrong number of "," characters'%line)
                    febNumber=int(addresses[0])
                    chXNumber=int(addresses[1])
                    crocXNumber=int(addresses[2])
                    sentMessageData=nregs*[0]
...
def DAQReadRcvMemReportFrames(iEvent, frms, theWriteType, theWFile, theFrame, theType, theIncludeCRC, theDataType23Hits):
    ...
    for frm in frms:
        ...
        if (theType=='CROC'  and (frm[1][1]&0xF0==0x20 and len(frm[1])==64)) or \
           ...
           (theType=='CROCE' and (frm[1][1]&0xF0==0x20 and len(frm[1])==64+2) and theIncludeCRC==True and theDataType23Hits==True)  or \
           (theType=='CROCE' and (frm[1][1]&0xF0==0x20 and len(frm[1])==68+2) and theIncludeCRC==True and theDataType23Hits==True):
            ...
            DAQBRAMReportHit(iEvent, hittrips, theWriteType, theWFile, theFrame)
             ...
             (theType=='CROCE' and theIncludeCRC==True  and (frm[1][1]&0xF0==0x30 and len(frm[1])%80==15)) or \
             (theType=='CROCE' and theIncludeCRC==True  and (frm[1][1]&0xF0==0x30 and len(frm[1])%40==17)):
