...
##            self.Bind(wx.EVT_BUTTON, self.OnFEDAQbtnAcqCtrlStop, self.frame.fe.daq.btnAcqCtrlStop)
    ...
    def PostToECL(self):
        ...
        out2 = "This is an automated posting by "+str(username)+"@"+str(hostname)+". A chain reset has been performed in SlowControl on crate "+str(self.frame.che.crateNumber)+" - CROCE "+str(self.frame.che.croceNumber)+" - chain "+str(self.frame.che.cheNumber)
    ...
    def OnMenuLoadHardwareWithChReset(self, event):
        ...
        #Add condition for which computerhost the command is run from. 
        #Want command to only happen from certain computers
        #Aaron said he'll show me how later. 
        file=open('ChRST_tracker.txt', 'r+')
        hostname = socket.gethostname() 
        username = getpass.getuser()
        out='This is an automated message. Find Hardware with CHAIN RESET has been performed by '+str(username)+'@'+str(hostname)
        file.write(out)
        ...
        e = ECLEntry(category='Automated_Entries/SlowControl',
                tags=['Auto-Entry'],
                formname='default',
                text=out, #"""This is beginning of the rise of the machines""",
                preformatted=False)
        elconn = ECLConnection(url="http://dbweb5.fnal.gov:8080/ECL/minerva", username="minerva_auto", password="axial2009")
        #
        # The user should be a special user created with the "raw password" by administrator.
