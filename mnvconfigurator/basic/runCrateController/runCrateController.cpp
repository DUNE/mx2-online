...
log4cpp::Category& runCrateController = log4cpp::Category::getInstance(std::string("runCrateController"));
...
// Pure Readback of FPGAs - Prints HV info regardless of debug level
...
// Basic Setup of FPGAs - not used, but here anyway...
...
int main(int argc, char *argv[]) 
{
	...
	runCrateController.setPriority(log4cpp::Priority::INFO);

	runCrateController.info("--Starting runCrateController script.--");
	...
	while(error==0) myController->GetCardStatus();
	//for(int i = 0; i < 1000; ++i) {
	//	myController->GetCardStatus();
	//	controllerLog.infoStream() << "reading the card status";		
	//}
...
#endif
		// Lets check to see if the deserializer & sync worked OK 
