...
log4cpp::Category& runCrocStatus = log4cpp::Category::getInstance(std::string("runCrocStatus"));
...
{
	//if (argc < 1) {
	//out << "Usage : runCrocStatus -c <CROC Address>\n";
	//	exit(0);
	//}
	...
	myAppender = new log4cpp::FileAppender("default", "/work/data/logs/crate0_readCrocStatus_log.txt");
	myAppender->setLayout(new log4cpp::BasicLayout());
	root.addAppender(myAppender);
	root.setPriority(log4cpp::Priority::ERROR);
	runCrocStatus.setPriority(log4cpp::Priority::INFO);

	runCrocStatus.info("--Starting runCrocStatus script.--");
	...
	//myController->MakeCroc(i<<16,(crocID));
	...
	while(error==0) {
		CROCClearStatusAndResetPointer(myController, myAcquire, myCroc);
		//std::cout << "reading croc " << ((myController->GetCroc(crocID)->GetCrocAddress())>>16) << std::endl;
	}
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
