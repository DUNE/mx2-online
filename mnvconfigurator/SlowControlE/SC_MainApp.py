...
##            self.Bind(wx.EVT_BUTTON, self.OnFEDAQbtnAcqCtrlStop, self.frame.fe.daq.btnAcqCtrlStop)
    ...
    def OnMenuActionsReadAllHV(self, event):
        try:
            ...
            if dlg.ShowModal()==wx.ID_OK:
                ...
                hvE=['FPGA:%s,%s,%s: Actual=%s, Target=%s, A-T=%s'% \
                    (dictHV['FPGA']['FEB'], dictHV['FPGA']['Channel'], dictHV['FPGA']['CROCE'], \
                    dictHV['Actual'], dictHV['Target'], dictHV['A-T']) for dictHV in hvEs]
                print '\n'.join(hv)
                print '\n'.join(hvE)
            dlg.Destroy()            
    ...
    def OnVMEbtnRunBoardTest(self, event):
        try:
            ...
            print '********************************'
            print '* CROCE BOARD TEST RUN RESULTS *'
            print '********************************'
            ...
            for theCROCE in self.vmeCROCEs:
                ...
                for itry in range(ntry):
                    ...
                    if self.frame.vme.BoardTest.chkTest1.IsChecked():
                        fails[1]=self.TestFastCommands(itry,theCROCE,fails[1])
                    ...
                    if self.frame.vme.BoardTest.chkTest2.IsChecked():
                        fails[2]=self.TestRSTTP(itry,theCROCE,fails[2],testTP=True,testRST=False,enCROCE=False,enCHE=False)
                        fails[2]=self.TestRSTTP(itry,theCROCE,fails[2],testTP=True,testRST=False,enCROCE=False,enCHE=True)
                        fails[2]=self.TestRSTTP(itry,theCROCE,fails[2],testTP=True,testRST=False,enCROCE=True,enCHE=False)
                        fails[2]=self.TestRSTTP(itry,theCROCE,fails[2],testTP=True,testRST=False,enCROCE=True,enCHE=True)
                    ...
                    if self.frame.vme.BoardTest.chkTest3.IsChecked():
                        fails[3]=self.TestRSTTP(itry,theCROCE,fails[3],testTP=False,testRST=True,enCROCE=False,enCHE=False)
                        fails[3]=self.TestRSTTP(itry,theCROCE,fails[3],testTP=False,testRST=True,enCROCE=False,enCHE=True)
                        fails[3]=self.TestRSTTP(itry,theCROCE,fails[3],testTP=False,testRST=True,enCROCE=True,enCHE=False)
                        fails[3]=self.TestRSTTP(itry,theCROCE,fails[3],testTP=False,testRST=True,enCROCE=True,enCHE=True)
                    ...
                    if self.frame.vme.BoardTest.chkTest4.IsChecked():#CAUTION: nwords>=nsend, nwords16%2==0 and SndMemIsFull if addr=1020
                        fails[4]=self.TestWSRfrmsFIFO(itry,theCROCE,fails[4],nwords16=508,nsend=64,
                            dw='D16',useBLT=False,useRAMMode=includeRAMMode,useRandom=useRandomData,theRAMData=theRAMData)
                    ...
                    if self.frame.vme.BoardTest.chkTest5.IsChecked():
                        fails[5]=self.TestWSRfrmsFIFO(itry,theCROCE,fails[5],nwords16=508,nsend=64,
                            dw='D32',useBLT=False,useRAMMode=includeRAMMode,useRandom=useRandomData,theRAMData=theRAMData)
                    ...
                    if self.frame.vme.BoardTest.chkTest6.IsChecked():
                        fails[6]=self.TestWSRfrmsFIFO(itry,theCROCE,fails[6],nwords16=508,nsend=64,
                            dw='D16',useBLT=True,useRAMMode=includeRAMMode,useRandom=useRandomData,theRAMData=theRAMData)
                    ...
                    if self.frame.vme.BoardTest.chkTest7.IsChecked():
                        fails[7]=self.TestWSRfrmsFIFO(itry,theCROCE,fails[7],nwords16=508,nsend=64,
                            dw='D32',useBLT=True,useRAMMode=includeRAMMode,useRandom=useRandomData,theRAMData=theRAMData)
                        self.frame.SetStatusText('%s: TEST#7: Memories D32 BLT: Fails=%s'%(theCROCE.Description(),fails[7]), 1)
                nfails=0
                for itest in range(1,len(fails)):
                    if fails[itest]!=0:
                        print '%s: TEST#%s FAIL %s times'%(theCROCE.Description(),itest,fails[itest])
                        nfails=nfails+fails[itest]
                if nfails==0:
                    print '*** %s: PASS ALL TESTS ***'%(theCROCE.Description())
                    self.frame.SetStatusText('*** %s: PASS ALL TESTS ***'%(theCROCE.Description()), 1)
                else:
                    print '%s: TOTAL FAILS %s times'%(theCROCE.Description(),nfails)
                    self.frame.SetStatusText('%s: TOTAL FAILS %s times'%(theCROCE.Description(),nfails), 1)
            print 'End  : %s'%time.ctime()
            print '********************************'
        except: ReportException('OnVMEbtnRunBoardTest', self.reportErrorChoice)
    def TestFastCommands(self,itry,theCROCE,fails):
        nfails=fails
        for fcmd in SC_Util.FastCmds:
            for iche in range(4):
                ...
                if data[2:]!=[0x0000,0x4040,0x2410,0x0000]: #FramesCounter,StatusFrame,StatusTXRX,RcvMemWPointer
                    print 'TRY#%s, TEST#1.1 Fast Commands=%s, %s:%s: Error ReadAllRegisters=%s, should be [0x0000,0x4040,0x2410,0x0000]'\
                        %(itry,fcmd.ljust(12),theCROCE.Description(),(theCROCE.Channels()[iche]).Description(),['0x'+hex(d)[2:].rjust(4,'0') for d in data[2:]])
                    nfails=nfails+1
            theCROCE.SendFastCommand(SC_Util.FastCmds[fcmd])
            for iche in range(4):
                if theCROCE.Channels()[iche].ReadStatusTXRX()!=0x2570:
                    print 'TRY#%s, TEST#1.2 Fast Commands=%s, %s:%s: Error TXRSStatus=%s, should be 0x2570'\
                        %(itry,fcmd.ljust(12),theCROCE.Description(),(theCROCE.Channels()[iche]).Description(),['0x'+hex(d)[2:].rjust(4,'0') for d in data])
                    nfails=nfails+1
        return nfails
    def TestRSTTP(self,itry,theCROCE,fails,testTP,testRST,enCROCE,enCHE,timeout=10):
        nfails=fails
        ...
        if data!=maskcroce:
            print 'TRY#%s, TEST#%s.1 %s, %s: Error RSTTP register: W=%s, R=%s'\
        ...
        for iche in range(4):
            ...
            if data&0xFC3F!=config:
                print 'TRY#%s, TEST#%s.2 %s, txtRAMData%s:%s: Error Configuration register: W=%s, R=%s'\
        ...
        for iche in range(4):
            ...
            if data!=txrxdata[iche]:
                print 'TRY#%s, TEST#%s.3 %s, %s:%s: Error TXRSStatus=%s, should be %s'\
                ...
                nfails=nfails+1
        return nfails
    def TestWSRfrmsFIFO(self,itry,theCROCE,fails,nwords16=508,nsend=64,dw='D16',useBLT=False,useRAMMode=True,useRandom=False,theRAMData=0xFFFF,timeout=100):
        #nword16=508=ok, nwords16=510=fail
        ...
        for iche in range(4):
            ...
            #set configuration (FIFO mode) and clear status
            ...
            if data!=[config,0x0000,0x0000,0x4040,0x2410,0x0000]: #Configuration,TriggerCounter,FramesCounter,StatusFrame,StatusTXRX,RcvMemWPointer
                print 'TRY#%s, TEST#%s.1 Write/Send/Receive Frames, %s:%s: Error ReadAllRegisters=%s, should be [%s,0x0000,0x0000,0x4040,0x2410,0x0000]'\
                    %(itry,tn,theCROCE.Description(),(theCROCE.Channels()[iche]).Description(),['0x'+hex(d)[2:].rjust(4,'0') for d in data],hex(config)[2:].rjust(4,'0'))
                nfails=nfails+1
            #write the message to SndMem in FIFO mode
            if useRandom==False: msgsnd=[(d&0x7FFF)|0x8000 for d in range(nwords16)]
            ...
            data=theCROCEChannelE.ReadAllRegisters()
            if data!=[config,0x0000,0x0000,0x0040,0x2410,0x0000]: 
                print 'TRY#%s, TEST#%s.2 Write/Send/Receive Frames, %s:%s: Error ReadAllRegisters=%s, should be [%s,0x0000,0x0000,0x0040,0x2410,0x0000]'\
                    %(itry,tn,theCROCE.Description(),(theCROCE.Channels()[iche]).Description(),['0x'+hex(d)[2:].rjust(4,'0') for d in data],hex(config)[2:].rjust(4,'0'))
                nfails=nfails+1
            #send all nsend messages
            for isend in range(nsend):
                #change the data[isend] from SndMem, in RAM mode, before sending. Do this only for isend>0 
                ...
                for i in range(timeout):
                    data=theCROCEChannelE.ReadAllRegisters()
                    if (data==[config,0x0000,isend+1,0x1010,0x2410,(4+2*nwords16)*(isend+1)]): break
                if (data!=[config,0x0000,isend+1,0x1010,0x2410,(4+2*nwords16)*(isend+1)]):
                    print 'TRY#%s, TEST#%s.3 Write/Send/Receive Frames, %s:%s: Error FrameNumber=%s, ReadAllRegisters=%s, should be [%s,0x0000,%s,0x1010,0x2410,%s]'\
                        %(itry,tn,theCROCE.Description(),theCROCEChannelE.Description(),isend,['0x'+hex(d)[2:].rjust(4,'0') for d in data],hex(config)[2:].rjust(4,'0'),'0x'+hex(isend+1)[2:].rjust(4,'0'),'0x'+hex((4+2*nwords16)*(isend+1))[2:].rjust(4,'0'))
                    nfails=nfails+1
            #read all nsend received messages (checking readout from ReceiveMemory)
            ibyte=0
            for isend in range(data[2]): #nsend frames
                rcvfrmData=[]
                #read frame number "isend"
                if dw=='D16':
                    rcvfrmsDataLengthBytes=theCROCEChannelE.ReadReceiveMemory(ibyte)
                    rcvfrmsStatus=theCROCEChannelE.ReadReceiveMemory(ibyte+2)
                    if useBLT==False:
                        for i16word in range(ibyte+4, ibyte+rcvfrmsDataLengthBytes, 2):
                            rcvfrmData.append(theCROCEChannelE.ReadReceiveMemory(i16word))
                    if useBLT==True:
                        data16=theCROCEChannelE.controller.ReadCycleBLT(theCROCEChannelE.RRcvMemory|(ibyte+4),rcvfrmsDataLengthBytes-4,am='A32_U_BLT',dw='D16sw')
                ...
                if dw=='D32':
                    ...
                    if useBLT==False:
                        for i32word in range(ibyte+4, ibyte+rcvfrmsDataLengthBytes, 4):
                    ...
                    if useBLT==True:
                        data32=theCROCEChannelE.controller.ReadCycleBLT(theCROCEChannelE.RRcvMemory|(ibyte+4),rcvfrmsDataLengthBytes-4,am='A32_U_BLT',dw='D32sw')
                        for i in range(0,len(data32),4):
                            rcvfrmData.append((data32[i+0]<<8) | data32[i+1])
                            rcvfrmData.append((data32[i+2]<<8) | data32[i+3])
                #check frame number "isend"
                if rcvfrmsDataLengthBytes!=4+2*nwords16:
                    print 'TRY#%s, TEST#%s.4 Write/Send/Receive Frames, %s:%s: Error FrameNumber=%s, RcvFrameLengthBytes=%s, should be %s'\
                        %(itry,tn,theCROCE.Description(),theCROCEChannelE.Description(),isend,rcvfrmsDataLengthBytes,4+2*nwords16)
                    nfails=nfails+1
                if rcvfrmsStatus!=0x1010:
                    print 'TRY#%s, TEST#%s.5 Write/Send/Receive Frames, %s:%s: Error FrameNumber=%s, RcvFrameStatus=%s, should be 0x1010'\
                        %(itry,tn,theCROCE.Description(),theCROCEChannelE.Description(),isend,hex(rcvfrmsStatus))
                    nfails=nfails+1
                if isend==0:
                    if rcvfrmData!=msgsnd:
                        print 'TRY#%s, TEST#%s.6 Write/Send/Receive Frames, %s:%s: Error FrameNumber=%s, \nSndFrameData=%s, \nRcvFrameData=%s'\
                            %(itry,tn,theCROCE.Description(),theCROCEChannelE.Description(),isend,[hex(d)[2:].rjust(4,'0') for d in msgsnd],[hex(d)[2:].rjust(4,'0') for d in rcvfrmData])
                        for i in range(len(rcvfrmData)):
                            if rcvfrmData[i]!=msgsnd[i]:
                                print '\tindex=%s, RcvFrameData=%s, expected=%s'%(i,hex(rcvfrmData[i]),hex(msgsnd[i]))
                        nfails=nfails+1
                else:                        
                    if (rcvfrmData[:isend]!=msgsnd[:isend] or rcvfrmData[isend+1:]!=msgsnd[isend+1:] or
                        ...
                        (useRAMMode==False and rcvfrmData[isend]!=msgsnd[isend])):
                            print 'TRY#%s, TEST#%s.7 Write/Send/Receive Frames, %s:%s: Error FrameNumber=%s, \nSndFrameData=%s, \nRcvFrameData=%s'\
                                %(itry,tn,theCROCE.Description(),theCROCEChannelE.Description(),isend,[hex(d)[2:].rjust(4,'0') for d in msgsnd],[hex(d)[2:].rjust(4,'0') for d in rcvfrmData])
                            for i in range(len(rcvfrmData)):
                                if i==isend and useRAMMode==True and rcvfrmData[i]!=theRAMData:
                                    print '\tindex=%s, RcvFrameData=%s, expected=%s'%(i,hex(rcvfrmData[i]),hex(theRAMData))
                                if i==isend and useRAMMode==False and rcvfrmData[i]!=msgsnd[i]:
                                    print '\tindex=%s, RcvFrameData=%s, expected=%s'%(i,hex(rcvfrmData[i]),hex(msgsnd[i]))
                                if not(i==isend) and rcvfrmData[i]!=msgsnd[i]:
                                    print '\tindex=%s, RcvFrameData=%s, expected=%s'%(i,hex(rcvfrmData[i]),hex(msgsnd[i]))
                            nfails=nfails+1
                #update pointer ibyte for next frame isend+1
                ibyte=ibyte+rcvfrmsDataLengthBytes
            #read all nsend frame pointers (checking readout from FramePointersMemory)
            frmPointersData=[]
            if dw=='D16':
                if useBLT==False:
            ...
            #check FramePointersMemory data
            for ifrmPointer in range(data[2]):
                if frmPointersData[ifrmPointer]!=ifrmPointer*2*(nwords16+2):
                    print 'TRY#%s, TEST#%s.8 Write/Send/Receive Frames, %s:%s: Error FrameNumber=%s, FramePointersMemoryData=%s'\
                        %(itry,tn,theCROCE.Description(),theCROCEChannelE.Description(),ifrmPointer,[hex(d)[2:].rjust(4,'0') for d in frmPointersData])
                    print '\tindex=%s, FramePointersMemoryData=%s, expected=%s'%(ifrmPointer,hex(frmPointersData[ifrmPointer]),hex(ifrmPointer*2*(nwords16+2)))
                    nfails=nfails+1  
        return nfails
    ...
    def OnFEFPGAbtnRead(self, event):
        try:
            ...
            rcvMessageData=theFEB.FPGARead(theCROCXChannelX, theType) 
    ...
    def OnFEFPGAbtnDumpRead(self, event):
        try:
            ...
            theCROCXChannelX=theCROCX.Channels()[self.frame.fe.chNumber]
            theFEB=FEB(self.frame.fe.febNumber)
            rcvMessageData=theFEB.FPGADumpRead(theCROCXChannelX, theType)
            theFEB.ParseMessageToFPGAtxtRegs(rcvMessageData, self.frame.fe.fpga.Registers.txtRegs)            
    ...
    def OnFEFPGAbtnWrite(self, event):
        try:
            ...
            rcvMessageData=theFEB.FPGAWrite(theCROCXChannelX, sentMessageData, theType)
            theFEB.ParseMessageToFPGAtxtRegs(rcvMessageData, self.frame.fe.fpga.Registers.txtRegs)            
    ...
    def OnFETRIPbtnRead(self, event):
        try:
            ...
            rcvMessageData=theFEB.TRIPRead(theCROCXChannelX, theTRIPIndex, theType)
    ...
    def OnFETRIPbtnRead6(self, event):
        try:
            ...
            theCROCXChannelX=theCROCX.Channels()[self.frame.fe.chNumber]
            theFEB=FEB(self.frame.fe.febNumber)
            rcvMessageData=theFEB.TRIPRead(theCROCXChannelX, None, theType)
    ...
    def OnFEFLASHbtnReadFlashToFile(self, event):
        try:
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                ReportException('OnFEFLASHbtnReadFlashToFile - Not yet implemented', self.reportErrorChoice)
                return
    ...
    def OnFEFLASHbtnCompareFileToFlash(self, event):
        try:
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                ReportException('OnFEFLASHbtnCompareFileToFlash - Not yet implemented', self.reportErrorChoice)
                return
            theCROC=FindVMEdev(self.vmeCROCs, self.frame.fe.crocNumber<<16)
            theCROCChannel=theCROC.Channels()[self.frame.fe.chNumber]
            theFEB=FEB(self.frame.fe.febNumber) 
    ...
    def OnFEFLASHbtnWriteFileToFlash(self, event):
        try:
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                ReportException('OnFEFLASHbtnWriteFileToFlash - Not yet implemented', self.reportErrorChoice)
                return
            theCROC=FindVMEdev(self.vmeCROCs, self.frame.fe.crocNumber<<16)
            theCROCChannel=theCROC.Channels()[self.frame.fe.chNumber]
            theFEB=FEB(self.frame.fe.febNumber)
    ...
    def OnFEFLASHbtnWriteFileToFlashThisCH(self, event):
        try:
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                ReportException('OnFEFLASHbtnWriteFileToFlashThisCH - Not yet implemented', self.reportErrorChoice)
                return
            theCROC=FindVMEdev(self.vmeCROCs, self.frame.fe.crocNumber<<16)
            theCROCChannel=theCROC.Channels()[self.frame.fe.chNumber]
            theFEB=FEB(self.frame.fe.febNumber)
    ...
    def OnFEFLASHbtnWriteFileToFlashThisCROC(self, event):
        try:
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                ReportException('OnFEFLASHbtnWriteFileToFlashThisCROC - Not yet implemented', self.reportErrorChoice)
                return
            theCROC=FindVMEdev(self.vmeCROCs, self.frame.fe.crocNumber<<16)
            theCROCChannel=theCROC.Channels()[self.frame.fe.chNumber]
            theFEB=FEB(self.frame.fe.febNumber)
    ...
    def OnFEFLASHbtnWriteFileToFlashALL(self, event):
        try:
            if self.frame.fe.TopLabels[2].GetValue()=='CHE' and self.frame.fe.TopLabels[4].GetValue()=='CROCE':
                ReportException('OnFEFLASHbtnWriteFileToFlashALL - Not yet implemented', self.reportErrorChoice)
                return
            theCROC=FindVMEdev(self.vmeCROCs, self.frame.fe.crocNumber<<16)
            theCROCChannel=theCROC.Channels()[self.frame.fe.chNumber]
            theFEB=FEB(self.frame.fe.febNumber)
