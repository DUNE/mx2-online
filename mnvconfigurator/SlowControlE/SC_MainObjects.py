...
class CROCEChannelE(VMEDevice):
    def __init__(self, cheNumber, baseAddr, controller):
        ...
        self.RegRRDFECounter           = self.cheBaseAddr + SC_Util.CROCECHERegs.RegRDFECounter
        self.RegRTXRstTpInDelayCounter = self.cheBaseAddr + SC_Util.CROCECHERegs.RegTXRstTpInDelayCounter
        self.RegRRcvMemFramesCounter   = self.cheBaseAddr + SC_Util.CROCECHERegs.RegRRcvMemFramesCounter
        self.RegRStatusFrame           = self.cheBaseAddr + SC_Util.CROCECHERegs.RegStatusFrame
        self.RegRStatusTXRX            = self.cheBaseAddr + SC_Util.CROCECHERegs.RegStatusTXRX
        self.RegRRcvMemWPointer        = self.cheBaseAddr + SC_Util.CROCECHERegs.RegRcvMemWPointer
        self.FEBs=[]
    ...
    def ReadRDFECounter(self): return int(self.controller.ReadCycle(self.RegRRDFECounter, am='A32_U_DATA', dw='D16'))
    def ReadTXRstTpInDelayCounter(self): return (0xFF00 & int(self.controller.ReadCycle(self.RegRTXRstTpInDelayCounter, am='A32_U_DATA', dw='D16')))>>8
    def ReadRcvMemFramesCounter(self): return 0x00FF & int(self.controller.ReadCycle(self.RegRRcvMemFramesCounter, am='A32_U_DATA', dw='D16'))
    ...
    def ReadRcvMemWPointer(self): return int(self.controller.ReadCycle(self.RegRRcvMemWPointer, am='A32_U_DATA', dw='D16'))
    ...
    def ReadFullDPMBLT(self):
        dpmPointer=self.ReadRcvMemWPointer()
...
class FEB():
    ...
    def FLASHDescription(self, thePageIndex, theCROCXChannelX, theCROCX):
        if theType=='CROC':
    ...
    def FLASHMainMemPageRead(self, theCROCChannel, pageAddr):
        ...
        rcvMessageData = WriteSendReceive(sentMessage, Frame.MessageDataLengthFLASHMMPRead, self.Address, Frame.DeviceFLASH, theCROCChannel, dw='D16', useBLT32=True)
    ...
    def FLASHMainMemPageProgThroughBuffer(self, theCROCChannel, pageAddr, pageBytes, bufferIndex=1):
        ...
        WriteSendReceive(sentMessage, Frame.MessageDataLengthFLASHMMPPTB, self.Address, Frame.DeviceFLASH, theCROCChannel, appendData=pageBytes[0], dw='D16', useBLT32=True)
    ...
    def AlignGateDelays(self, theCROCX, theCROCXChannelX, theNumberOfMeas, theLoadTimerValue, theGateStartValue, theType='CROC'):
        ...
        for febAddress in theCROCXChannelX.FEBs:
            theFEB=FEB(febAddress)
            rcvMessage=theFEB.FPGARead(theCROCXChannelX, theType)
...
def WriteSendReceiveCROCE(sentMessage, rcvMessageLength, theFEBAddress, theFEBDevice, theCROCEChannelE, appendData=0, dw='D16', useBLT32=False):
    ...
    if useBLT32: rcvMessageHeader, rcvMessageData = GetRcvMessageHeaderAndDataBLTCROCE(theCROCEChannelE)
    else: rcvMessageHeader, rcvMessageData = GetRcvMessageHeaderAndDataCROCE(theCROCEChannelE,dw)
    ###print [hex(x)[2:].rjust(2,'0') for x in sentMessage], len(sentMessage)
    ###print [hex(x)[2:].rjust(2,'0') for x in rcvMessageHeader], len(rcvMessageHeader)
    ###print [hex(x)[2:].rjust(2,'0') for x in rcvMessageData], len(rcvMessageData)
    if rcvMessageLength!=0 and len(rcvMessageData)!=rcvMessageLength: raise Exception(
        'Error rcvDataLength expected=%s, rcv=%s'%(rcvMessageLength, len(rcvMessageData)))
    return rcvMessageData
...
def ClearAndCheckStatusRegisterCROCE(theCROCEChannelE, chk=True):
    ...
    if chk:
        ...
        if data[2:]!=[0x0000,0x4040,0x2410,0x0000]: raise Exception(
            '%s: Error ReadAllRegisters=%s, should be [0x00XX,0xXXXX,0x0000,0x4040,0x2410,0x0000]'\
...
def WriteFIFOAndCheckStatusCROCE(theMessage, theCROCEChannelE, appendData=0, chk=True):
    ...
    if chk:
        data=theCROCEChannelE.ReadAllRegisters() # check FramesCounter,StatusFrame,StatusTXRX,RcvMemWPointer
        if data[2:]!=[0x0000,0x0040,0x2410,0x0000]: raise Exception(
            '%s: Error ReadAllRegisters=%s, should be [0x00XX,0xXXXX,0x0000,0x0040,0x2410,0x0000]'\
            %(theCROCEChannelE.Description(),['0x'+hex(d)[2:].rjust(4,'0') for d in data]))
...
def SendFIFOAndCheckStatusCROCE(theCROCEChannelE, chk=True):
    ...
    if (data[2:5]!=[0x0001,0x1010,0x2410]): raise Exception(
        '%s: Error ReadAllRegisters=%s, should be [0x00XX,0xXXXX,0x0001,0x1010,0x2410,0xXXXX]'\
        %(theCROCEChannelE.Description(),['0x'+hex(d)[2:].rjust(4,'0') for d in data]))
...
def GetRcvMessageHeaderAndDataCROCE(theCROCEChannelE,dw='D16'):
    RcvMemWPointerRegister=theCROCEChannelE.ReadRcvMemWPointer()
    ...
    if rcvMessageLength<(4+Frame.MessageHeaderLength): raise Exception(
        '%s: Error RcvMemMessageLength=%s < 4 + %s (message too short)'\
    ...
    rcvMessageHeader=rcvMessage[4:13]
    if rcvMessageLength>13: rcvMessageData=rcvMessage[13:rcvMessageLength]
...
class WorkerThreadCROCE(threading.Thread):
    ...
    def run(self):
        ...
        for iEvent in range(self.nEvents):
            ...
            #frms=SC_MainMethods.DAQSpitRcvmemInFrames(rcvmem, 'CROCE')
