            ...
            for theCROCEChannelE in theCROCE.Channels():
                ...
                for febAddr in range(1,16):
                    for itry in range(1,5):
                        ...
                        if data!=[data[0]&0x03C0,0x0000,0x0000,0x4040,0x2410,0x0000,theHeader]:
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error1 ReadAllRegisters1=%s, should be [0x%s,0x0000,0x0000,0x4040,0x2410,0x0000,0x%s]'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(), 
                                  ['0x'+hex(d)[2:].rjust(4,'0') for d in data],
                                  hex(data[0]&0x03C0)[2:].rjust(4,'0'),hex(theHeader)[2:].rjust(4,'0'))
                            fails=fails+1
                        #write message and check registers
                        sndmsg=[febAddr<<8]
                        theCROCEChannelE.WriteSendMemory(sndmsg[0])
                        data=theCROCEChannelE.ReadAllRegisters()
                        if data!=[data[0]&0x03C0,0x0000,0x0000,0x0040,0x2410,0x0000,theHeader]:
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error2 ReadAllRegisters2=%s, should be [0x%s,0x0000,0x0000,0x0040,0x2410,0x0006,0x%s]'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(),
                                  ['0x'+hex(d)[2:].rjust(4,'0') for d in data],
                                  hex(data[0]&0x03C0)[2:].rjust(4,'0'),hex(theHeader)[2:].rjust(4,'0'))
                            fails=fails+1
                        ...
                        data=theCROCEChannelE.ReadAllRegisters()
                        if ((data[:-2]!=[data[0]&0x03C0,0x0000,0x0001,0x1010,0x2410]) or
                            (data[-2]!=0x000C and data[-2]!=0x0014) or data[-1]!=theHeader):
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error3 ReadAllRegisters3=%s, should be [0x%s0,0x0000,0x0001,0x1010,0x2410,0x000C/14,0x%s]'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(),
                                  ['0x'+hex(d)[2:].rjust(4,'0') for d in data],
                                  hex(data[0]&0x03C0)[2:].rjust(4,'0'),hex(theHeader)[2:].rjust(4,'0'))
                            fails=fails+1
                        ...
                        if rcvfrmsDataLengthBytes!=0x000C and rcvfrmsDataLengthBytes!=0x0014:
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error4 RcvMemLengthBytes=%s, should be 0x000C or 0x0014'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(),
                                  rcvfrmsDataLengthBytes)
                            fails=fails+1
                        if rcvfrmsStatus!=0x1010:
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error5 RcvMemStatus=0x%s, should be 0x1010'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(),
                        ...
                        if rcvfrmsFirmwareDevFunc!=((febvers<<8)+(devfunc)):
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error6 rcvfrmsFirmwareDevFunc=0x%s, should be 0x%s'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(),
                                  hex(rcvfrmsFirmwareDevFunc)[2:].rjust(4,'0'),hex((crateid<<14)+(croceid<<9))[2:].rjust(4,'0'))
                            fails=fails+1
                        if rcvfrmsSourceID!=sourceid:
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error7 rcvfrmsSourceID=0x%s, should be 0x%s'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(),
                                  hex(rcvfrmsSourceID)[2:].rjust(4,'0'),hex(sourceid)[2:].rjust(4,'0'))
                            fails=fails+1
                        if rcvfrmsDataLengthBytes!=rcvfrmsDataLengthBytes2:
                            print 'TRY#%s, FindCROCEFEBs, %s:%s: Error8 RcvMemLengthBytes=%s, RcvMemLengthBytes2=%s'\
                                %(itry,theCROCE.Description(),theCROCEChannelE.Description(),
...
def DAQReadRcvMemReportFrames(iEvent, frms, theWriteType, theWFile, theFrame, theType):
    ...
    for frm in frms:
        ...
        if (theType=='CROC'  and (frm[1][1]&0xF0==0x20 and len(frm[1])==64)) or \
           (theType=='CROCE' and (frm[1][1]&0xF0==0x20 and len(frm[1])==64)):       #FPGA Device FPGAREG Frame (read or write)     
            ...
            DAQBRAMReportNotDecodedFrame(iEvent, frm, theWriteType, theWFile, theFrame)
             ...
             (theType=='CROCE' and ((frm[1][1]&0xF0==0x10 and len(frm[1])==630) or \
                                  (frm[1][1]&0xF0==0x10 and len(frm[1])==734))):    #Trip Device TRIPREG Frame Read=630, Write=734
            if frm[1][0]&0x80==0x80: direction='S'          #frame comming from Slave (FEB) 
            else: direction='M'                             #frame comming from Master (CROCChannel) 
            febAddress=frm[1][0]&0x0F                       #retrieve febAddress
            if frm[1][0]&0x70==0x00 and frm[1][1]&0x0F==0x03 and frm[1][2]==0x03: #no frame header errors
            ...
            tripindex='?'
            if theType=='CROCE':
                theDevice=(frm[2][5]&0xF0)>>4
                theFunctn=(frm[2][5]&0x0F)
                conv_dev3func_to_tripindex=['?','?','?','?','?','?','?','?','0','1','2','3','4','5','?','?']
                if theDevice==3: tripindex=conv_dev3func_to_tripindex[theFunctn]
            triphits=theFEB.DecodeBRAMTrips(frm[1][9:], frm[0], tripindex)
            #theFrame.fe.daq.display.AppendText('\ntriphits=%s'%(triphits))
            ...
            hittrips=theFEB.DecodeBRAMHits(frm[1][9:], frm[0], hitindex)
            #theFrame.fe.daq.display.AppendText('\nhittrips=%s'%(hittrips))
...
def BRAMReadDiscrim(theCROCXDescription, theCROCXChannelX, theFEB, theType):
    ...
    source='%s:%s:%s:%s'%(theCROCXDescription, theCROCXChannelX.Description(), SC_Util.VMEdevTypes.FE, theFEB.Address)
...
def BRAMReadTrip(theCROCXDescription, theCROCXChannelX, theFEB, index, theType):
    rcvMessageData,rcvMFH_10bytes=theFEB.BRAMReadTrip(theCROCXChannelX, index, theType)
    source='%s:%s:%s:%s'%(theCROCXDescription, theCROCXChannelX.Description(), SC_Util.VMEdevTypes.FE, theFEB.Address)
...
def BRAMReadHit(theCROCXDescription, theCROCXChannelX, theFEB, index, theType):
    rcvMessageData,rcvMFH_10bytes=theFEB.BRAMReadHit(theCROCXChannelX, index, theType)
    source='%s:%s:%s:%s'%(theCROCXDescription, theCROCXChannelX.Description(), SC_Util.VMEdevTypes.FE, theFEB.Address)
...
def workerDAQSimple(nEvents, theLock, theStopEvent, theCTRLChannel, theCTRL, theCTRLs, theFrame, theReadType, theWriteType, theWFile):
    ...
    while iEvent<nEvents:
        ...
        try:
            ...
            frms=DAQSplitRcvmemInFrames(rcvmem, 'CROCE')
