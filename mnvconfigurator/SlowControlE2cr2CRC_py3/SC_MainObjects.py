...
##                    theLength=Frame.MessageDataLengthBRAMTripRead24Hith   #(zero+data+zero)
    ...
    def ParseMessageToFPGAtxtRegs(self, msg, txtRegs):
        ...
        #message word 54: WR Spare 8b v95
        if len(msg)==55 and msg[32]==95: #message word 32: R  Firmware Version, 8 bits
            txtRegs[56].SetValue(hex(msg[54])[2:].upper().rjust(2,'0'))
        else:
            txtRegs[56].SetValue('00')
    def ParseFPGARegsToMessage(sel, txtRegs):
        ...
        # 03/30/2015 Geoff Savage
	# Make the maximum HV Pulse Width 45, was 30
        if pw<=45: msg[37]=pw
        else: msg[37]=45        
        #if pw<=30: msg[37]=pw
        #else: msg[37]=30        
        ...
        #message word 54: WR Spare 8b v95
        if Frame.NRegsFPGA==55:
            msg[54] = (int(txtRegs[56].GetValue(),16)) & 0xFF
        return msg
...
class Frame():
    ...
    NRegsFPGA=55                            #was 54 in v91, now is 55 in v95
    NRegsTRIPLogical=20
...
##    MessageDataLengthBRAMDiscrimRead=1281   #FRED=2bytes*20records*8hits*4trips=1280+dummy=1281
    ...
    def GetReceivedHeaderErrors(self, header, direction, broadcast, febAddress, device, MFHeader,frameID0=0, frameID1=0):
        err=[]
        ...
        if len(err)>0:
          err.append(GetEAddress(MFHeader))
        return err

def GetEAddress(MFHeader):
    sourceID=MFHeader[6]<<8|MFHeader[7]
    crateId = (sourceID >> 14) & 0x1
    crocId = (sourceID >> 9) & 0xF
    chanId = (sourceID >> 7) & 0x3
    febId = (sourceID >> 3) & 0xF
    return "Address:%d-%d-%d-%d"%(crateId,crocId,chanId,febId)
...
def WriteSendReceive(sentMessage, rcvMessageLength, theFEBAddress, theFEBDevice, theCROCChannel,
    ...
    rcvHeaderErr=Frame().GetReceivedHeaderErrors(rcvMessageHeader,Frame.DirectionS2M, Frame.BroadcastNone,
        theFEBAddress, theFEBDevice, rcvMFH_10bytes)
...
lastMFH=""#Global variable for debug
...
##          (theCROCEChannelE.Description(),dw,useBLT,includeCRC,readNbytes)
    ...
    strMFH=""
    for i in rcvMessageMFH:
      strMFH="%s %s"%(strMFH,hex(i))

    MFHMsg="Previous MFH %s\n Current MFH %s"%(strMFH,lastMFH)
    ...
    if rcvMessageMFHLength1!=rcvMessageMFHLength2: raise Exception(
        ...
        %(theCROCEChannelE.Description(),'0x'+hex(rcvMessageMFHLength1)[2:].rjust(4,'0'),\
          '0x'+hex(rcvMessageMFHLength2)[2:].rjust(4,'0'),MFHMsg))
    if rcvMessageMFHLength1!=RcvMemWPointerRegister: raise Exception(
        ...
        %(theCROCEChannelE.Description(),'0x'+hex(RcvMemWPointerRegister)[2:].rjust(4,'0'),\
          '0x'+hex(rcvMessageMFHLength1)[2:].rjust(4,'0'),MFHMsg))
    if rcvMessageMFHStatus!=StatusFrameRegister: raise Exception(
        ...
        %(theCROCEChannelE.Description(),'0x'+hex(StatusFrameRegister)[2:].rjust(4,'0'),\
          '0x'+hex(rcvMessageMFHStatus)[2:].rjust(4,'0'),MFHMsg))
    ...
    if RcvMemWPointerRegister<(10+Frame.MessageHeaderLength): raise Exception(
        ...
        %(theCROCEChannelE.Description(),RcvMemWPointerRegister,Frame.MessageHeaderLength,MFHMsg))
