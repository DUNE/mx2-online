...
##            self.Bind(wx.EVT_BUTTON, self.OnFEDAQbtnAcqCtrlStop, self.frame.fe.daq.btnAcqCtrlStop)
    ...
    def OnMenuActionsAbout(self, event):
        try:
            ...
            print('* Version : %s Release Date: December 18 2014                           *'%(self.version))
            print('*------------------------------------------------------------------------------*')
...
##                        if fails[8]!=0: return
                    ...
                    if self.frame.vme.BoardTest.chkTest9.IsChecked():
                        ...
                        for iche in theCheckCh:
                            rdfeCounters[iche]=theCROCE.Channels()[iche].ReadRDFECounter()              #2
                        ...
                        for iche in theCheckCh:
                            ...
                            rcvmem=theCROCE.Channels()[iche].ReadFullDPMBLT() # returns a multiple of four bytes
                            ##print 'itry=%d: %s:%s: len(rcvmem)=%s, rcvmem=%s'%\
                            ##      (itry,theCROCE.Description(),theCROCE.Channels()[iche].Description(),\
                            ##       len(rcvmem),''.join([hex(w16)[2:].rjust(4,'0') for w16 in rcvmem]))
                            #length correction based on theCROCE.includeCRC==False/True
                            #the '3' means 1discrim+1hit+1fpgaregisters=3frames
                            ...
                            #checking rcvmem length
                            #NOTE: data content is not checked since there is no 'real' frame data
                            if len(rcvmem)!=lenRcvMemRead:
                                errmsg='TRY#%s, TEST#9.2: Sequencer, %s:%s: ERROR ReceiveMem length=%s, should be %s'\
                                    %(itry,theCROCE.Description(),theCROCE.Channels()[iche].Description(),len(rcvmem),lenRcvMemRead)
                                fails[9]=fails[9]+1
                                if self.daqWFile!=None: self.daqWFile.write('\n'+errmsg)
                                else: print(errmsg)
...
##                                (nfebs!=0 and statusFrame!=0x5410)):
                            if ((nfebs==0 and statusFrame!=0x4440 and statusFrame!=0x5450 \
                                          and statusFrame!=0x0440 and statusFrame!=0x1450) or
                                (nfebs!=0 and statusFrame!=0x5410 and statusFrame!=0x1410)):
                                ...
                                errmsg='TRY#%s, TEST#9.4: Sequencer, %s:%s: ERROR StatusFrame=%s, should be 0x4440/0x5450/0x5410 / 0x0440/0x1450/0x1410'%(itry,
                            ...
                            if rcvMemWPointer!=lenRcvMem:
                                errmsg='TRY#%s, TEST#9.6: Sequencer, %s:%s: ERROR RcvMemWPointer=%s, should be %s'%(itry,
                                    theCROCE.Description(),theCROCE.Channels()[iche].Description(),rcvMemWPointer,lenRcvMem)
                                fails[9]=fails[9]+1
                                if self.daqWFile!=None: self.daqWFile.write('\n'+errmsg)
                                else: print(errmsg)
...
##                                    else: print errmsg
                    ...
                    if theCheckCRC==True and rcvfrmCRCs!=[]:
                        theCRCs=0
                        #print '#7.5.1. isend=%d rcvfrmRead=%s'%(isend,[hex(d)[2:].rjust(4,'0') for d in rcvfrmRead])
                        #print '#7.5.2. isend=%d rcvfrmData=%s'%(isend,[hex(d)[2:].rjust(4,'0') for d in rcvfrmData])
                        #print '#7.5.3. isend=%d rcvfrmCRCs=%s'%(isend,[hex(d)[2:].rjust(4,'0') for d in rcvfrmCRCs])
                        theCRCbits=[1,1,1,1,1,1,1,1]
                        for w in rcvfrmData:
                        ...
                        theCRCs=theCRCByte<<8
                        for w in rcvfrmRead[:5]:
                            theCRCbits=newCRC((0xFF00&w)>>8,theCRCbits)
                            theCRCbits=newCRC(0x00FF&w,theCRCbits)
                        theCRCByte=0
                        for i in range(8):
                            theCRCByte=theCRCByte+theCRCbits[i]*(2**i)
                        theCRCs=theCRCs+theCRCByte
