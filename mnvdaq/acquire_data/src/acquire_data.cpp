...
// (certainly interrupt resets must be handled by the DAQ).
	...
	acqData.debug("  CRIM Timing Setup    = 0x%04X", daqController->GetCrim(index)->GetTimingSetup());
	acqData.debug("  CRIM GateWidth Setup = 0x%04X", daqController->GetCrim(index)->GetGateWidthSetup());
	acqData.debug("  CRIM TCALB Setup     = 0x%04X", daqController->GetCrim(index)->GetTCALBPulse());
 ...
 */
	...
	acqData.debug("  Timing Register Address  = 0x%X\n",daqController->GetCroc(crocNo)->GetTimingAddress());
	acqData.debug("  Timing Register Message  = 0x%X\n",daqController->GetCroc(crocNo)->GetTimingRegister());
	acqData.debug("  Timing Message (Sending) = 0x%02X%02X\n",croc_message[1],croc_message[0]);
	try {
	...
	acqData.infoStream() << " Using " << nChannels << " FE Channels per CROC.";
#endif
 ...
 */
	...
	int nFEBperChannel = 15; // maximum number of FEB's per channel
 ...
 */
	{ // Debug statements
		...
		if (evt->feb_info[4] != 3) {
			std::cout << " Embedded Event Data Length is:     " << (unsigned int)evt->event_data[0] + 
				(unsigned int)(evt->event_data[1]<<8) << std::endl;
			for (int i = 0; i < 11; i++) {
				std::cout << "  evt->event_data[" << i << "] = " << (unsigned int)evt->event_data[i] 
				<< std::endl;
			}
		}
#endif
	...
	while (et_alive(sys_id)) {
		...
		if (status == ET_ERROR_DEAD) {
			...
			printf("Got an ET timeout!\n");
			break;
		} else if (status == ET_ERROR_EMPTY) {
			printf("No ET events!\n");
			break;
		} else if (status == ET_ERROR_BUSY) {
			printf("ET Grandcentral is busy!\n");
			break;
		} else if (status == ET_ERROR_WAKEUP) {
			printf("ET wakeup error!\n");
			break;
		} else if ((status == ET_ERROR_WRITE) || (status == ET_ERROR_READ)) {
			printf("ET socket communication error!\n");
			break;
		} if (status != ET_OK) {
			printf("ET et_producer: error in et_event_new!\n");
			exit(0);
		} 
