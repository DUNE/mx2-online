...
private:
	/* Flagged for removal
	const int MaxHits;
	unsigned short adc[3], tdc; //I'm making the potetially errenous assumption
	//that these are the adc & tdc values
	int channelsPerTrip; //should make this more "consty"
	*/
	unsigned char *buffer; /*!<A buffer for holding ADC raw data */  
						//a copy of the buffer that comes off of the system for the ADC's

public: 
	// /*! The simple constructor which takes a log file output streamer argument */
	// adc(std::ofstream &log_file):Frames(log_file) { }; 
	 adc(febAddresses a, RAMFunctionsHit f, std::ofstream &log_file); //must be sent an FEB number for addressing
	/*! The specialized constructor which makes up the read frame */
	//adc(febAddresses a, RAMFunctionsHit f); //must be sent an FEB number for addressing
...
private:
	int MaxHits;
	unsigned char *buffer; //a copy of the buffer that comes off of the system for the discr

public: 
	// /*! The simple constructor which takes a log file output streamer argument */
	// disc(std::ofstream &log_file):Frames(log_file) { }; 
	/*    disc(febAddresses a, RAMFunctionsChip f); //must be sent an FEB number for addressing
	disc(febAddresses a, RAMFunctionsHit f); //must be sent an FEB number for addressing */
	 disc(febAddresses a,std::ofstream &log_file); // discriminator always takes the same RAMFunction
	/*! The specialized constructor which makes up the read frame */
	//disc(febAddresses a); // discriminator always takes the same RAMFunction, outgoing message is always the same!
	/*! The default destructor */
