...
// adctdc::adctdc(febAddresses a, RAMFunctionsChip b) : MaxHits(8), channelsPerTrip(36) {
//   /*********************************************************************************
//  * a: The address (number) of the feb
//  * b: The "RAM Function" which describes the chip to be read off
//  *********************************************************************************/
...
adc::adc(febAddresses a, RAMFunctionsHit b, std::ofstream &log_file): Frames(log_file) {
 ...
 *********************************************************************************/
  ...
  Broadcasts broad = None; //we don't broadcast
  Directions dir = MasterToSlave; //ALL outgoing messages are master-to-slave
  MakeDeviceFrameTransmit(dev, broad, dir,(unsigned int)b, (unsigned int) febNumber[0]); //make up the frame
 ...
 */
	...
	// First, show the adc in simple sequential order...
		...
		// So, is "channel mask" 00<32>00 or 0<32>000 or 000<32>0?
		// Casper uses 3 skip channels per trip to unpack, so probably: 000<32>0
	...
	// Show the adc keyed by pixel value...
		...
		const int nPixelsPerFEB         = 64;
    		//unused// const int nHiMedChannelsPerPixel= 2;
    		const int nHiMedTripsPerFEB     = 4;
    		const int nSides                = 2;
    		//unused// const int nHiMedTripsPerSide    = nHiMedTripsPerFEB/nSides;
    		//unused// const int nLoTripsPerFEB        = 2;
    		...
    		//unused// const int nWordsPerChannel      = 2; // charge + time
    		//unused// const int nBytesPerWord         = 2; // "word" means unsigned short
    		// Pixel map for the weirdly arranged low-gain channels
...
disc::disc(febAddresses a,std::ofstream &log_file):Frames(log_file) {
// may want to eventually make this a two arg function for algorithmic convenience
 ...
 *********************************************************************************/
  ...
  #if DEBUG_FEB
    std::cout<<"Made DISC!: "<<OutgoingMessageLength<<" Device: "<<dev<<std::endl;
  #endif
}
...
{
/*! \fn 
 * decodes the registers (so we can get the number of hits to process)
 * argument is dummy for now...
