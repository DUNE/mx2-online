...
trips::trips(febAddresses a, TRiPFunctions f, int maxHits, std::ofstream &log_file):Frames(log_file) {
//trips::trips(febAddresses a, TRiPFunctions f, int maxHits) {
  /*! \fn ********************************************************************************
 ...
 *********************************************************************************/
  TripTChipID[0]=0x0A; //the id number for the trip; for reasons I don't know, they're all 10
  ...
  Broadcasts broad = None; //we don't ever broadcast to/from the trips (as far as I know)
  Directions dir = MasterToSlave; //out-going is always master-to-slave
  MakeDeviceFrameTransmit(dev, broad, dir,(unsigned int)f, (unsigned int) febNumber[0]);   //make up the FPGA frame
  trip_function = (unsigned char)f; //cast the trip function
  read = false; //the default position is that we are going to write information to a trip(s)
  // for (int i=0;i<15;i++) { //loop over the 15 possible values to be sent to the trips
  //   trip_values[i]=(long_m) 0; //initialize the values
  // } 
  //set the 15 values to "default" values 
  trip_values[0] = (long_m) 50; //60; //ibp
  trip_values[1] = (long_m) 120; //ibpfoll2
  trip_values[2] = (long_m) 0; //iff
  trip_values[3] = (long_m) 160; //ibpiff1ref
  trip_values[4] = (long_m) 255; //ibpopamp
  trip_values[5] = (long_m) 0; //ib_t
  trip_values[6] = (long_m) 0; //iffp2
  trip_values[7] = (long_m) 20; //ibcomp
  trip_values[8] = (long_m) 180; //v_ref
  trip_values[9] = (long_m) 0; //v_th
  trip_values[10] = (long_m)(2*maxHits - 1); //pipe delay
  trip_values[12] = (long_m) 3; //irsel
  trip_values[13] = (long_m) 3; //iwsel
  trip_values[14] = (long_m) 0; //inject
  // Different GAIN register settings for low-gain TRiPs (or at least there can be)
  switch (f) {
    case tTR4: //low gain
      trip_values[11] = (long_m) 11; //gain
      break;
    case tTR5: //low gain
      trip_values[11] = (long_m) 11; //gain
      break;
    default: //tTR0->tTR3 are high/medium gain trips.
      trip_values[11] = (long_m) 11; //gain
      break;
  }
}

void trips::EncodeRegisterValues() {
  /*! \fn 
 *
 ...
 *        */
  ...
  std::vector<unsigned char> packTripData;  //we need some flexibility in the packing array,
                                     //so we'll use an STL list for packing procedure
...
#if DEBUG_FEB
	std::cout << "In trips::EncodeRegisterValues... PHYSICAL REGISTERS:" << std::endl;
   	for (int i=0;i<14;i++) {
		if ( (i!=12) && (i!=13) ) {
       		std::cout<<"trip_register: "<<i+1<<" "<<(int)trip_registers[i]<<std::endl;
		} else if (i==12) {
       		std::cout<<"trip_register: "<<i+1<<" "<<(int)trip_registers[i]<<" (unused)"<<std::endl;			
		} else if (i==13) {
       		std::cout<<"trip_register: "<<i+1<<" "<<(int)trip_registers[i]<<" (inject)"<<std::endl;			
		}
    }
#endif
...
void trips::PackBits(bool control, bool clock, bool data, bool reset, std::vector<unsigned char> &l) {
  /*! \fn 
 * note we're setting one bit of a string of data bits into a for bits of an 8 bit bit string to 
 *  encode the entire bit string of the piece of data.  Confused yet? 
 *  */
...
void trips::SortNSet(bool reset, long_m data, int bits, bool control, bool lowToHigh, std::vector<unsigned char> &l) {
  /*! \fn
  ...
  if (!error) { //if there are none, we're off to the races
    int startByte = 11; //remember that ALL incoming headers are 11 bytes long 
      ...
      for (int j=0;j<14;j++) { // loop here is really over *physical* registers (1...12), note j+1's below..
        if ((j==12)||(j==13)) continue; //register 12 (13 in trip-land) isn't used by us
        /* it would seem that this stuff is some kind of error checking. 
...
void trips::GetRegisterValue(int j, int &index, int bits) {
  /*! \fn ********************************************************************************
...
void trips::ParseError(int i, int j) {
  /*! \fn ********************************************************************************
 ...
 *********************************************************************************/
  std::cout<<"Parse Error for register: "<<i
      <<" at index: "<<j<<std::endl;
  exit(1);
}

void trips::ParseError(int i) {
  /*! \fn ********************************************************************************
 *  This function raises an error if the message being processed contains
 *  "bad" data.
 *
 ...
 *********************************************************************************/
  std::cout<<"Parse Error at index: "<<i
      <<" while getting register value."<<std::endl;
  exit(1);
}

void trips::MakeMessage() {
  /*! \fn ********************************************************************************
 ...
 *********************************************************************************/
   ...
   for (int i=0;i<messageSize; i++) {  // bufferSize? 
