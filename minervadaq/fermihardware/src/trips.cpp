...
#if DEBUG_FEB
  tripsLog.debugStream() << "In trips::EncodeRegisterValues... PHYSICAL REGISTERS:";
  for (int i=0;i<14;i++) {
    if ( (i!=12) && (i!=13) ) {
      tripsLog.debugStream()<<"trip_register: "<<i+1<<" "<<(int)trip_registers[i];
    } else if (i==12) {
      tripsLog.debugStream()<<"trip_register: "<<i+1<<" "<<(int)trip_registers[i]<<" (unused)";			
    } else if (i==13) {
      tripsLog.debugStream()<<"trip_register: "<<i+1<<" "<<(int)trip_registers[i]<<" (inject)";			
    }
  }
#endif
...
void trips::SortNSet(bool reset, long_m data, int bits, bool control, bool lowToHigh, 
...
int trips::DecodeRegisterValues(int messageLength) 
{
  ...
  if (!error) { //if there are none, we're off to the races
    ...
    tripsLog.debugStream() << "Errors Found in Trip Message";
    return 1;
  }
  return 0;
}
...
void trips::ParseError(int i, int j) 
{
   ...
   */
  tripsLog.debugStream() << "Parse Error for register: " << i << " at index: " << j;
  exit(EXIT_FEB_UNSPECIFIED_ERROR);
}
...
void trips::ParseError(int i) 
{
   ...
   */
  tripsLog.debugStream() << "Parse Error at index: " << i << " while getting register value.";
  exit(EXIT_FEB_UNSPECIFIED_ERROR);
}
