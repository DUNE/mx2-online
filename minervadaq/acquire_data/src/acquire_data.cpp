...
#endif
#if WH14T
  detectorString        = "WH14 Top Crate. (minervatest02)";
  std::cout            << "Initializing hardware for the " << detectorString << std::endl; 
  acqData.infoStream() << "Initializing hardware for the " << detectorString; 
  InitializeCrim(0xE00000, 1, runningMode);
...
#endif
#if WH14B
  detectorString        = "WH14 Bottom Crate. (minervatest04)";
  std::cout            << "Initializing hardware for the " << detectorString << std::endl; 
  acqData.infoStream() << "Initializing hardware for the " << detectorString; 
  InitializeCrim(0xE00000, 1, runningMode);
  InitializeCroc(0x010000, 1, 2, 0, 0, 0);
    ...
    const bool readFPGA, const int nReadoutADC, const bool zeroSuppress)
{
  ...
  for (int i=0; i<(*rop0)->getDataLength(); i++) {
#if DEBUG_NEWREADOUT                    
    acqData.debugStream() << "reference channel address    = " << (*rop0)->getChannel(i)->GetChannelAddress();
  ...
  // Note that by reading the discriminators first we can choose not to read other 
  // frame types on boards without hits (we could also refuse to declare hitless 
  // discriminator frames to ET).
  ...
  if (readFPGA && continueReadout) {
    ...
    while (rop != readoutObjects->end() && continueReadout) {
      ...
      for (int i=0; i<(*rop)->getDataLength(); i++) {
#if DEBUG_NEWREADOUT
        acqData.debugStream() << "  Sending clear and reset on channel " << 
      ...
      for (int i=0; i<(*rop)->getDataLength(); i++) {
        ...
        if (doReadout) {
          ...
          evt->feb_info[3] = (*rop)->getChannel(i)->GetChainNumber();
          evt->feb_info[6] = brdnum;
          // We don't parse the FPGA's anymore in the DAQ... (int)tmpFEB->GetFirmwareVersion();
          evt->feb_info[7] = firmwareVersion; 
          // FillEventStructure here.  FES reads from the *channel's* buffer, not the frame's!
          FillEventStructure(evt, 2, (*rop)->getChannel(i));
          // ContactEventBuilder here.
          ContactEventBuilder(evt, (int)0, attach, sys_id); 
          // Cleanup... (buffer created in RecvFrameData
          (*rop)->getChannel(i)->DeleteBuffer();
          tmpFEB = 0;
        }
        // Re-set do-read.
