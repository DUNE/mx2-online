"""
  ...
  with the run control.
   ...
   Original author: A. Mislivec (mislivec@fnal.gov)
                    Feb.-Mar. 2010
"""
...
class MyFrame(wx.Frame):
	...
	def UpdateHWConfig(self,evt):
		for run in self.runSeries.Runs:
			run.hwConfig = MetaData.HardwareConfigurations.code(self.hardwareConfigEntry.GetStringSelection())
	...
	def WriteLEDGroups(self,run):
		leds = MetaData.LEDGroups.description(run.ledGroup)
	...
	def GetRunFromGUI(self):
		...
		runMode  = MetaData.RunningModes.hash(self.runModeEntry.GetStringSelection())
		hwConfig = MetaData.HardwareConfigurations.hash(self.hardwareConfigEntry.GetStringSelection())
		ledLevel = "--"
		leds     = "--"

		if self.isLIRunMode(runMode):
			ledLevel = MetaData.LILevels.hash(self.ledLevelEntry.GetStringSelection())
			leds = MetaData.LEDGroups.hash(self.ReadLEDGroups())
	...
	def UpdateGUIFromRun(self,run):
		...
		self.runModeEntry.SetSelection(self.runModeEntry.FindString(MetaData.RunningModes.description(run.runMode)))
		self.hardwareConfigEntry.SetSelection(self.hardwareConfigEntry.FindString(MetaData.HardwareConfigurations.description(run.hwConfig)))

		if self.isLIRunMode(run.runMode):
			self.ledLevelEntry.SetSelection(self.ledLevelEntry.FindString(MetaData.LILevels.description(run.ledLevel)))
