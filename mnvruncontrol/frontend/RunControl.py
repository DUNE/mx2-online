...
class MainFrame(wx.Frame):
	...
	def BuildGraphics(self):
		...
		self.autoStartSeriesEntry = optionsMenu.Append(-1, "Auto-start next run series", "Automatically start the next run series when the previous one finishes cleanly.", kind=wx.ITEM_CHECK)
	...
	def StopRunning(self, evt=None):
		""" Stops the data acquisition sequence and re-enables the controls.
		    Invoked either as the event handler for the "button" event
		    generated by clicking the red "Stop" button or the "StopRunning"
		    event generated by the DataAcquisitionManager when it finishes a run. """
		    
		if self.runmanager.running:
			wx.PostEvent(self.runmanager, Events.StopRunningEvent())
#			self.runmanager.StopDataAcquisition()
			return							# StopDataAcquisition() will issue another event when it's done
		
		# an "auto" StopRunning event is one that's generated
		# when a run series finishes normally.
		# if this is a run series (not a single subrun),
		# and if the user has specified to do so in the options menu,
		# we continue on to the next run series.
		if hasattr(evt, "auto") and evt.auto and self.runSeriesButton.GetValue() == True \
		   and self.autoStartSeriesEntry.IsChecked():

			self.runEntry.SetValue(self.runEntry.GetValue() + 1)
			self.runEntry.SetRange(self.runEntry.GetValue(), int(1e9))
			self.subrunEntry.SetValue(1)
			wx.PostEvent(self, Events.StartRunningEvent())
			return
...
#		self.logfileLocation                 = Configuration.params["Master node"]["master_logfileLocation"]
	...
	def GetLastRunConfig(self, evt=None):
		...
		self.runEntry.SetRange(key_values["run"], int(1e9))
		...
		self.autoStartSeriesEntry.Check(key_values["autostartseries"])
	...
	def StoreLastRunConfig(self, evt=None):
		...
		try:
			...
			db["autostartseries"] = self.autoStartSeriesEntry.IsChecked()
