...
#		self.subrun = 0
	...
	def StartNextSubrun(self, evt=None):
		...
		# get the hardware configuration from the runinfo.
		# if
		if not quitting:
...
#			self.hwconfigfile = "NOFILE"
			...
			if self.runinfo.hwConfig != MetaData.HardwareConfigurations["Current state"] and (self.subrun == 0 or len(self.runseries.Runs) == 1 or self.runinfo.hwConfig != self.runseries.Runs[subrun - 1].hwConfig):
				for node in self.readoutNodes:
					...
					if not success:
						wx.PostEvent( self.main_window, Events.ErrorMsgEvent(text="Could not configure the hardware for the " + node.name + " readout node.  This subrun will be aborted.", title="Hardware configuration problem") )
		...
		# check if the voltages and HV periods are obviously ok.
		# if not, the user needs to confirm that we should
		...
		self.StartNextThread()			# starts the first thread.  the rest will be started in turn as SIGUSR1 signals are received by the program.
